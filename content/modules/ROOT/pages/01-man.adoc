...
The following command will list all `man` pages available on your system:
...
[TIP]
====
The first time you try to list `man` pages, you may need to update your
...
Try tab completion for yourself by copying the following
command, without the full `grep` keyword spelled out, Then, press tab.
...
Since there wasn't a unique match for `grep` pressing tab a second time
brings up the list of possible complettions. Depending on your 
terminal config, you may also get a bell or visual indicator. 

.Output
[source,text]
----
grab                       groupmod                   grub2-get-kernel-settings  grub2-script-check
grep                       groups                     grub2-install              grub2-set-bootflag
grid                       growpart                   grub2-menulst2cfg          grub2-set-default
groff                      grpck                      grub2-mkconfig             grub2-set-password
grops                      grpconv                    grub2-mkimage              grub2-setpassword
grotty                     grpunconv                  grub2-mkpasswd-pbkdf2      grub2-switch-to-blscfg
group.conf                 grub2-bios-setup           grub2-mkrelpath            grubby
groupadd                   grub2-editenv              grub2-mount                
groupdel                   grub2-emu                  grub2-probe                
groupmems                  grub2-file                 grub2-reboot 
----

From the list, `grep` is the only option with `e` as the next letter, add an
`e` and hit `tab` again. You should see `grep` fill in with an extra space after it.
Some commands allow tab completion throughout a series of arguments. You can test
this with commands like `nmcli c s` by hitting tab after the `c` and `s`.
====
=====
