...
[source,text]
----
Fri Mar  7 05:17:10 PM EDT 2025
----
...
since that is the default for the lab systems. 

Converting a date and time to epoch time is as simple as a single argument 
(`+%s`) with `date`. You can also specify the `-d` option to input a 
datetime string instead of using the current time.

.Command
[source,bash,subs="+macros,+attributes",role=execute]
----
date -d "Dec 12 2014 15:00:00" +%s
----
...
This has been a small sample of quirks of Linux systems. You can learn more 
helpful commands and tips in our other labs.
