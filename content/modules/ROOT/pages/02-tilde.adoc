...
This command has no expected output. However, you can see that the end of 
the prompt has updated. By default, the `bash` prompt includes the name of
...
----
authorized_keys  config  {guid}key.pem  {guid}key.pub
----
...
directory, using the print working directory (`pwd`) command:
