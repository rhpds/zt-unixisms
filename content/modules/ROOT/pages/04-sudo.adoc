...
perform administrative tasks. Privileges can be assigned to system groups, individual users, and in other ways.
Traditionally, users in the _wheel_ group are admins and have elevated access via the `sudo` command. 
...
You can get information about your user and group memberships via the `id` command.
...
----
uid=1000(rhel) gid=1000(rhel) groups=1000(rhel),10(wheel) context=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023
----
